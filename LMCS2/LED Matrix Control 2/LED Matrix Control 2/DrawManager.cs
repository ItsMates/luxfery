...
namespace LED_Matrix_Control_2
{
    class DrawManager
    {
        ...
        public DrawObject DrawPixel(MouseEventArgs e)
        {
            ...
            else if (x == previousHoverLoc.X && y == previousHoverLoc.Y)
            {
                form.pbm.boxes[x, y].BackColor = Color.FromArgb(127,127,127,127);
            }
            else
            {
                form.pbm.boxes[previousHoverLoc.X, previousHoverLoc.Y].BackColor = actualColors[previousHoverLoc.X, previousHoverLoc.Y];
