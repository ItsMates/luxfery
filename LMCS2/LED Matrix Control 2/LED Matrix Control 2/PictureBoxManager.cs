...
namespace LED_Matrix_Control_2
{
    public class PictureBoxManager
    {
        ...
        {
            int size = Properties.Settings.Default.boxSize;
            int size_max = Properties.Settings.Default.boxSizeMax;
            ...
            for (int y = 0; y < loadedHeight; y++)
            {
                for (int x = 0; x < loadedWidth; x++)
                {
                    ...
                    {
                        Location = new Point((int)((x * size) / pixelSpacing) + 3, (int)((y * size) / pixelSpacing) + 3),
                        Width = pixelSize - 1,
        ...
        public void SendPixel(int x, int y, byte[] data)
        {
            ...
            int rawIndex = x + y * Properties.Settings.Default.pixelsX;
